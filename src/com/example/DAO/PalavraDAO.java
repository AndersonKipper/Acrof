...
public class PalavraDAO extends SQLiteOpenHelper {
	...
	public void cadastrarPalavras(SQLiteDatabase database) {
		try {
			...
			// Instancia do stringbuilder que sera utilizada para leitura do
			// arquivo
			// sb = new StringBuilder();

			while ((insert = br.readLine()) != null) {
				database.execSQL(insert);
				// Toast.makeText(c, "Numero de registros " + insert ,
			...
			Toast.makeText(c, "Excecao no arquivo " + e1.getMessage(),
	...
	public String getPalavra(String nivel, ArrayList<String> categoria) {
		String sql;
		if (categoria.size() > 0) {
			int c = new Random().nextInt(categoria.size());
			
			sql = "SELECT palavra, categoria FROM " + TABELA + " WHERE nivel = '" + nivel
					+ "' AND categoria = '" + categoria.get(c) + "'";
		...
		try {
			while (cursor.moveToNext()) {
				...
				if (i == id) {
					...
					setCategoria(cursor.getString(1));
					// Toast.makeText(c, sql + " --- " + palavra,
					// Toast.LENGTH_LONG).show();
				}
				i++;
			}
